...
export default function EditBookingScreen() {
  const { bookingId, hotelId } = useLocalSearchParams<{ bookingId: string; hotelId: string }>();
  ...
  const loadData = async () => {
    try {
      const [bookings, ag, rt] = await Promise.all([
        api.get<Booking[]>(`/hotels/${hotelId}/bookings`),
      ...
      if (booking) {
        setForm({
          ...
          rooms: booking.rooms?.toString() || "1",
          status: booking.status || "confirmed",
          agencyId: booking.agencyId?.toString() || "",
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await api.put(`/hotels/${hotelId}/bookings/${bookingId}`, {
        ...form,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.background }}>
      ...
      >
        ...
        <SelectField label="Room Type *" value={form.roomType} options={roomTypeOptions} onChange={(v) => set("roomType", v)} placeholder="Select room type" />
        ...
        <SelectField label="Agency" value={form.agencyId} options={agencyOptions} onChange={(v) => set("agencyId", v)} />
        ...
        <Pressable
          ...
          disabled={saving}
        >
          {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>Update Booking</Text>}
        </Pressable>
      </KeyboardAwareScrollView>
    </View>
  );
