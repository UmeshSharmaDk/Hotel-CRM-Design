...
}: { booking: Booking; onEdit: () => void; onDelete: () => void }) {
      ...
      <View style={styles.cardFooter}>
        ...
        <View style={styles.costSection}>
          ...
          {parseFloat(booking.dueBalance) > 0 && (
            <Text style={styles.dueBalance}>Due: ₹{parseFloat(booking.dueBalance).toLocaleString("en-IN")}</Text>
          )}
        </View>
        <Pressable
...
export default function BookingsScreen() {
  ...
  const params = useLocalSearchParams<{ filterDate?: string; filterType?: string }>();
  ...
  // Apply dashboard params on initial navigation
  useEffect(() => {
  ...
  const activeFilterCount = [filterMonth, filterYear, filterAgency].filter(Boolean).length;
  ...
  let filtered = bookings.filter((b) => {
    ...
    // Dashboard filterDate + filterType
    if (params.filterDate && params.filterType === "checkin" && b.checkIn !== params.filterDate) return false;
    if (params.filterDate && params.filterType === "checkout" && b.checkOut !== params.filterDate) return false;
  ...
  filtered = [...filtered].sort((a, b) => {
    switch (sortBy) {
      case "checkin_desc": return new Date(b.checkIn).getTime() - new Date(a.checkIn).getTime();
      case "checkin_asc": return new Date(a.checkIn).getTime() - new Date(b.checkIn).getTime();
      case "checkout_desc": return new Date(b.checkOut).getTime() - new Date(a.checkOut).getTime();
      case "checkout_asc": return new Date(a.checkOut).getTime() - new Date(b.checkOut).getTime();
      default: return 0;
    }
  });
    ...
    <View style={[styles.screen, { paddingTop: topPad }]}>
      ...
      <View style={styles.header}>
        <View>
          ...
          {params.filterDate && params.filterType && (
            <Text style={styles.filterHint}>
              {params.filterType === "checkin" ? "Check-ins" : "Check-outs"} for {formatDate(params.filterDate)}
              {"  "}
              <Text style={{ color: C.primary }} onPress={() => router.setParams({ filterDate: "", filterType: "" })}>
                Clear
              </Text>
            </Text>
          )}
        ...
        <View style={styles.headerActions}>
          ...
          >
            <Feather name="sliders" size={16} color={showFilters ? C.primary : C.textSecondary} />
            {activeFilterCount > 0 && (
      ...
      {showFilters && (
        <View style={styles.filterPanel}>
          <View style={styles.filterRow}>
            <View style={{ flex: 1 }}>
              <SelectField label="Month" value={filterMonth} options={MONTHS} onChange={setFilterMonth} />
            </View>
            <View style={{ flex: 1 }}>
              <SelectField label="Year" value={filterYear} options={YEARS} onChange={setFilterYear} />
            </View>
          </View>
          <SelectField label="Agency" value={filterAgency} options={agencyOptions} onChange={setFilterAgency} />
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <BookingCard
              booking={item}
              onEdit={() => router.push({ pathname: "/booking/edit", params: { bookingId: item.id, hotelId: selectedHotelId } })}
...
const styles = StyleSheet.create({
  ...
  filterHint: { fontSize: 12, fontFamily: "Inter_400Regular", color: C.textSecondary, marginTop: 2 },
  ...
  filterToggleBtnActive: { backgroundColor: C.primaryLight, borderColor: C.primary },
  filterBadge: {
  ...
  searchWrap: {
    ...
    backgroundColor: C.card, marginHorizontal: 16, marginTop: 10, marginBottom: 4,
    paddingHorizontal: 14, paddingVertical: 10,
    borderRadius: 12, borderWidth: 1, borderColor: C.border, gap: 8,
  },
  ...
  filterLabel: { fontSize: 13, fontFamily: "Inter_600SemiBold", color: C.text, marginBottom: 8, marginTop: 4 },
  ...
  sortChipText: { fontSize: 12, fontFamily: "Inter_500Medium", color: C.textSecondary },
  ...
  clearFiltersText: { fontSize: 13, fontFamily: "Inter_500Medium", color: C.danger },
  countRow: {
    paddingHorizontal: 16, paddingVertical: 6,
  },
  countText: { fontSize: 12, fontFamily: "Inter_400Regular", color: C.textMuted },
  ...
  cardDates: {
    ...
    backgroundColor: C.background, borderRadius: 8, padding: 10, marginBottom: 10,
  },
  ...
  agencyBadge: {
    ...
    backgroundColor: C.primaryLight, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 20,
  },
